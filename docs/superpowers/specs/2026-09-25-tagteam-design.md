...
| Joining groups | 6-digit numeric code, **single use**, generated by any member, shared manually; unused codes expire after 7 days |
...
- `invite_code` — code (6 digits, unique among active), groupId, createdBy, expiresAt (7 days),
  usedBy, usedAt, revokedAt. Single use: consumed atomically on join. Join attempts rate limited
  (5/min, 20/h per user) + per-IP.
